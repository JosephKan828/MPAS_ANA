...
def main():
    ...
    dispersion = EquatorialWaveDispersion( # type: ignore
    ...
    # Compute phase speed of kelvin wave
    ...
    cntl_speed = np.nansum((frm / wnm * (2*np.pi*6.371e6/86400)) * symm_vint["CNTL"] * kel_mask) / np.nansum(symm_vint["CNTL"] * kel_mask)
    ncrf_speed = np.nansum((frm / wnm * (2*np.pi*6.371e6/86400)) * symm_vint["NCRF"] * kel_mask) / np.nansum(symm_vint["NCRF"] * kel_mask)
    ...
    plt.title(f"CNTL: {cntl_speed:.2f} m/s")
    ...
    plt.xlabel("Zoanl Wavenumber")
    plt.ylabel("Frequency [ CPD ]")
    plt.title(f"NCRF: {ncrf_speed:.2f} m/s")
    plt.colorbar(c, label="Normalized Power Spectrum",
                    orientation="horizontal")
